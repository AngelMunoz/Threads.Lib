...
module Media =
    ...
    module Owner =
        let Decode: Decoder<Owner> =
            Decode.object (fun get -> { id = get.Required.Field "id" Decode.string })
    ...
    module ThreadChildren =
        let Decode: Decoder<ThreadChildren> =
            Decode.object (fun get -> { data = get.Required.Field "data" (Decode.list ThreadId.Decode) })
    ...
    let getThreads baseUrl accessToken pagination threadFields cancellationToken =
        task {
            let fields =
                threadFields |> Seq.map ThreadField.asString |> (fun f -> String.Join(",", f))
    ...
    let getThread baseUrl accessToken threadId threadFields cancellationToken =
        task {
            let fields =
                threadFields |> Seq.map ThreadField.asString |> (fun f -> String.Join(",", f))
