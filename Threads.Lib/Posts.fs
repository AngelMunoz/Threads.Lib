...
module Posts =
    ...
    let createSingleContainer baseUrl accessToken userId postParams cancellationToken =
        taskResult {
            let postParams =
                postParams
                |> Seq.filter (fun f ->
            ...
            let! req =
                http {
                    ...
                    query [ yield! postParams; "is_carousel_item", "false"; "access_token", accessToken ]
                }
                |> Request.sendTAsync

            let! (response: PostId) = req |> Response.deserializeJsonTAsync cancellationToken
    ...
    let createCarouselItemContainer baseUrl accessToken userId postParams cancellationToken =
        taskResult {
            let postParams =
                postParams
                |> Seq.filter (fun f ->
            ...
            let! req =
                http {
                    POST $"{baseUrl}/{userId}/threads"
                    query [ yield! postParams; "is_carousel_item", "true"; "access_token", accessToken ]
                }
                |> Request.sendTAsync
    ...
    let createCarouselContainer baseUrl accessToken userId children textContent cancellationToken =
        taskResult {
    ...
    let publishContainer baseUrl accessToken userId containerId cancellationToken =
        task {
            let! req =
                http {
